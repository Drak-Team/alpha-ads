...
const Plans = () => {
  ...
  const handleConfirmDeposit = async () => {
    ...
    setLoading(true);
    try {
      // احمد بھائی، یہاں سے میں نے 'User Check' ہٹا دیا ہے تاکہ ایرر نہ آئے
      const fileName = `deposit-${Date.now()}-${Math.random().toString(36).substring(7)}.${file.name.split('.').pop()}`;

      // بالٹی کا نام وہی ہے جو آپ نے سپا بیس میں بنایا
    ...
    <div className="min-h-screen bg-[#064e3b] text-white p-4 pb-24 font-sans text-right">
        ...
        {plans.map((plan) => (
          <div key={plan.name} className={`relative overflow-hidden bg-gradient-to-l ${plan.color} to-white/5 border border-white/10 p-5 rounded-[32px] flex justify-between items-center shadow-xl`}>
            ...
            <div className="z-10 text-right">
              ...
              <p className="text-[10px] opacity-70 mt-1 font-urdu">روزانہ آمدنی: ${plan.daily}</p>
            </div>
      ...
      {selectedPlan && (
        <div className="fixed inset-0 bg-black/95 flex items-center justify-center p-5 z-50 backdrop-blur-sm">
          <div className="bg-[#064e3b] border-2 border-yellow-500/50 w-full max-w-sm rounded-[40px] p-8 relative shadow-2xl text-right">
            ...
            <div className="text-center mb-6">
              ...
              <p className="text-xs opacity-60 mt-1 font-urdu">رقم بھیجیں اور اسکرین شاٹ لگائیں</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <button onClick={handleConfirmDeposit} disabled={loading} className={`w-full py-4 rounded-2xl font-black text-lg ${loading ? 'bg-gray-700' : 'bg-green-600 hover:bg-green-500'}`}>
