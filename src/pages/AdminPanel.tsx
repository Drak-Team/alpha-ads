...
const AdminPanel = () => {
  ...
  const handleWithdrawalAction = async (id: string, newStatus: 'approved' | 'rejected') => {
    ...
    if (!error) { alert(newStatus === 'approved' ? 'Approved ✅' : 'Rejected ❌'); fetchAll(); }
  };
  ...
  const handleDepositAction = async (id: string, newStatus: 'approved' | 'rejected') => {
    ...
    if (!error) { alert(newStatus === 'approved' ? 'Approved — Balance auto updated ✅' : 'Rejected ❌'); fetchAll(); }
  };
  ...
  if (!isAdmin) return <div className="min-h-screen bg-[#0d0a1a] flex items-center justify-center text-white">Loading...</div>;
    ...
    <div className="min-h-screen bg-[#0d0a1a] text-white p-4 pb-28">
      ...
      <div className="flex gap-2 mb-6">
        {(['deposits', 'withdrawals', 'users'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)} className={`flex-1 py-3 rounded-2xl font-bold text-xs ${tab === t ? 'bg-yellow-500 text-[#0d0a1a]' : 'bg-[#1a1035] border border-purple-500/10'}`}>
      ...
      <div className="space-y-4">
        {loading ? <p className="text-center opacity-50 py-10">Loading...</p>
        : tab === 'users' ? (
          <>
            <div className="relative mb-4">
              <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 opacity-40" />
              <input type="text" placeholder="Search by name, phone or code..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
                className="w-full bg-[#1a1035] border border-purple-500/10 rounded-2xl py-3 pl-10 pr-4 text-sm placeholder:opacity-40 focus:outline-none focus:border-yellow-500/50" />
            </div>
            {filteredUsers.length === 0 ? <p className="text-center opacity-30 py-10">No users found</p>
            : filteredUsers.map(u => {
              const userSubs = getUserSubs(u.id);
              return (
                <div key={u.id} className="bg-[#1a1035] border border-purple-500/10 rounded-2xl p-4">
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <div className="text-right">
                      <p className="text-green-400 font-black text-sm">PKR {u.balance}</p>
                      <p className="text-yellow-400 font-bold text-[10px]">Earned: PKR {u.total_earned}</p>
                      <p className="text-[9px] opacity-40">Ref: PKR {u.referral_earnings}</p>
                    </div>
                    ...
                    <div className="mt-3 pt-3 border-t border-white/10 space-y-2">
                      ...
                      {userSubs.map(s => (
                        <div key={s.id} className={`flex justify-between items-center text-[11px] p-2 rounded-xl ${s.is_active ? 'bg-green-500/10 border border-green-500/20' : 'bg-white/5 border border-white/5'}`}>
                          ...
                          <span className="text-[10px] opacity-60">PKR {(s as any).plans?.price} | PKR {(s as any).plans?.daily_earning}/day</span>
                  ...
                  {adjustingUser === u.id && (
                    <div className="mt-3 pt-3 border-t border-white/10">
                      <input type="number" placeholder="Enter amount" value={adjustAmount} onChange={e => setAdjustAmount(e.target.value)}
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <button onClick={() => handleBalanceAdjust(u.id, 'add')} className="bg-green-600 py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-1 active:scale-95">
                          <Plus size={14} /> Add
                        </button>
                        <button onClick={() => handleBalanceAdjust(u.id, 'subtract')} className="bg-red-600/20 text-red-400 py-2 rounded-xl font-bold text-xs flex items-center justify-center gap-1 border border-red-500/20 active:scale-95">
          ...
          deposits.length === 0 ? <p className="text-center opacity-30 py-10">No pending deposits</p> : (
            deposits.map(dep => (
              <div key={dep.id} className="bg-[#1a1035] border border-purple-500/10 p-5 rounded-2xl">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => handleDepositAction(dep.id, 'approved')} className="bg-green-600 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95">
                    <CheckCircle size={18} /> Approve
                  </button>
                  <button onClick={() => handleDepositAction(dep.id, 'rejected')} className="bg-red-600/20 text-red-500 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 border border-red-500/20 active:scale-95">
          ...
          withdrawals.length === 0 ? <p className="text-center opacity-30 py-10">No pending withdrawals</p> : (
            withdrawals.map(req => (
              <div key={req.id} className="bg-[#1a1035] border border-purple-500/10 p-5 rounded-2xl">
                <div className="flex justify-between items-center mb-3">
                  <span className="bg-yellow-600 text-[#0d0a1a] px-3 py-1 rounded-full text-[10px] font-black uppercase">{req.method}</span>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <button onClick={() => handleWithdrawalAction(req.id, 'approved')} className="bg-green-600 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 active:scale-95">
                    <CheckCircle size={18} /> Approve
                  </button>
                  <button onClick={() => handleWithdrawalAction(req.id, 'rejected')} className="bg-red-600/20 text-red-500 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 border border-red-500/20 active:scale-95">
                    <XCircle size={18} /> Reject
                  </button>
                </div>
              </div>
