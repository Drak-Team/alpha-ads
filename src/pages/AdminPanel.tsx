...
import { CheckCircle, XCircle, Image, ArrowLeft, Shield, Users, Search, Plus, Minus } from 'lucide-react';
...
const AdminPanel = () => {
  ...
  const fetchAll = async () => {
    setLoading(true);
    const [{ data: wData }, { data: dData }, { data: uData }] = await Promise.all([
        ...
        {loading ? (
          ...
          <>
            <div className="relative mb-4">
              ...
              <input
                type="text"
                placeholder="نام، فون یا کوڈ سے تلاش کریں..."
                value={searchQuery}
            ...
            <div className="bg-[#1a3a32] border border-white/10 rounded-2xl p-3 mb-2 grid grid-cols-12 text-[10px] font-bold opacity-50 uppercase">
              <span className="col-span-4">نام</span>
              <span className="col-span-3 text-center">بیلنس</span>
              <span className="col-span-3 text-center">کمائی</span>
              <span className="col-span-2 text-center">ایکشن</span>
            </div>
              ...
              filteredUsers.map(u => (
                <div key={u.id} className="bg-[#1a3a32] border border-white/10 rounded-2xl p-4">
                  <div className="grid grid-cols-12 items-center gap-1">
                    ...
                    <div className="col-span-3 text-center">
                      <p className="text-yellow-400 font-bold text-xs">₨{u.total_earned}</p>
                      <p className="text-[9px] opacity-40">Ref: ₨{u.referral_earnings}</p>
                    </div>
                    <div className="col-span-2 flex justify-center">
                      <button
                        onClick={() => setAdjustingUser(adjustingUser === u.id ? null : u.id)}
                        className="bg-yellow-500/20 text-yellow-500 p-2 rounded-xl text-[10px] font-bold"
                      >
                        ₨±
                      </button>
                    </div>
